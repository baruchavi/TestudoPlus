chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        if (request.type == "prof") {
        ...
        else if (request.type == "grades") {
            url = `https://planetterp.com/api/v1/grades?course=${request.course}`
            ...
            caches.match(url).then((response) => {
                ...
                if (response !== undefined && parseInt(response.headers.get("sw-fetched-on"))+expTime > new Date().getTime()) {
                    ...
                    console.log("fetching for "+request.prof+" @ url "+`https://planetterp.com/api/v1/grades?course=${request.course}`)
                    fetch(`https://planetterp.com/api/v1/grades?course=${request.course}`)
                    .then((response) => {
                        let responseClone = response.clone();
                        ...
                        caches.open("v1").then((cache) => {
                            ...
                            responseClone.blob().then(body=>{
                                ...
                                cache.put(`https://planetterp.com/api/v1/grades?course=${request.course}`, newresp);
                            })
                        });
                        response.json().then(json => {
                            sendResponse({error: false, data: json})
